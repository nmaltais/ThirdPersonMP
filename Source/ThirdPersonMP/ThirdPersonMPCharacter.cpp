...
{
    float healthLeft = CurrentHealth - DamageTaken;
    SetCurrentHealth(healthLeft);
    if(healthLeft <= 0){
        Die();
    }
    return healthLeft;
...
void AThirdPersonMPCharacter::Die() {
    // Respawn after 3 seconds
    AController* PC = Controller;
    AThirdPersonMPGameMode* GM = Cast<AThirdPersonMPGameMode>(GetWorld()->GetAuthGameMode());
    if (GM) {
        GM->RespawnPlayerWithDelay(PC, 3);
    }
    Destroy();
}
